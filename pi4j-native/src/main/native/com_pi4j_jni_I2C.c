...
{
    int response = ioctl(fd, I2C_SLAVE, deviceAddress); 	// Set the port options and set the address of the device we wish to speak to
  ...
  (JNIEnv *env, jobject obj, jint fd, jint deviceAddress, jint localAddress)
{
    ...
    buf[0] = localAddress;												// This is the register we wish to read from to get software version and battery volts
